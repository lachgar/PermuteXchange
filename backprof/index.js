...
app.post('/professeurs', async (req, res) => {
  try {
    ...
    const { nom, prenom, tel, email, grade, specialite, faculteActuelle, villeFaculteActuelle, villeDesiree, password } = req.body;
...
app.post('/password/init', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Mot de passe initialisé avec succès' });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Une erreur est survenue lors de l\'initialisation du mot de passe' });
  }
});
